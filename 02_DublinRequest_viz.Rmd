...
sg.labels <- data.frame(
  iso2 = c("DE", "CH", "SE", "XX"),
  x = rep(as.Date("2015-03-27"), 4),
  y = c(30000, 57000, 70000, 94500)
)
...
for(lang in colnames(txt)) {
  ...
  # PLOT streamgraph
  sg <- streamgraph(ddd, group, values, time, right = 13, left = 50) %>% 
  sg_axis_x(tick_interval = 1, "year", tick_format = "%Y") %>%
  sg_fill_brewer("PuBu") %>%
  sg_annotate(label = labels$label[1], x = labels$x[1], y = labels$y[1], color = "grey") %>%
  sg_annotate(label = labels$label[2], x = labels$x[2], y = labels$y[2], color = "grey") %>%
  sg_annotate(label = labels$label[3], x = labels$x[3], y = labels$y[3], color = "grey") %>%
  sg_annotate(label = labels$label[4], x = labels$x[4], y = labels$y[4], color = "grey")
...
regions <- iso2.sub
names(regions) <- rep('#2b5555', length(regions))
